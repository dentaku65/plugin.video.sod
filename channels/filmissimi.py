...
def elenco(item):
    ...
    for scrapedthumbnail,scrapedurl,scrapedtitle in scrapedSingle(item.url,'<ul class="recent-posts">(.*?)</ul>',patron):
          ...
          itemlist.append(infoSod(channel=__channel__, action="findvideos", title=scrapedtitle,titlein="[COLOR azure]",titleend="[/COLOR]", url=scrapedurl,thumbnail=scrapedthumbnail, tipo="movie"))

    # Paginazione
    # ===========================================================================================================================
    matches=scrapedSingle(item.url,'<div class="navigation">(.*?)</div>',"current'>.*?</span>.*?class='page-numbers'.*?href='(.*?)'>.*?</a>")
    if len(matches) > 0:
        paginaurl = matches[0]
